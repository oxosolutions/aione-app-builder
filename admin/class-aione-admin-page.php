...
abstract class Aione_Admin_Page
{
    ...
    public function add_meta_boxes()
    {
         ...
         * @param type  $var Description.
         */
        do_action('aione_closedpostboxes', $screen->base);
    ...
    protected function submitdiv($button_text, $form = array(), $type = 'custom-post-type', $built_in = false )
    {
        //if ( WPCF_Roles::user_can_edit($type, $this->ct) ) {
            ...
            if(
                ( isset( $_GET['group_id'] ) || isset( $_GET['aione-tax'] ) )
                && isset( $_GET['page'] )
                && ! $built_in
            ) {
                switch( $_GET['page'] ) {
                    case 'aione-edit':           // post fields
                        $action = 'delete_group';
                        break;
                    case 'aione-edit-usermeta':  // user fields
                        $action = 'delete_usermeta_group';
                        break;
                    case 'aione-termmeta-edit':  // term fields
                        $action = 'delete_term_group';
                        break;
                    case 'aione-edit-tax':       // taxonomy
                ...
                if( isset( $action ) ) {
                    $args = array(
                        ...
                        'aione_warning' => urlencode(__('Are you sure?', 'aione-app-builder'))
                    );

                    if( isset( $_GET['group_id'] ) ) {
                        $args['group_id'] = sanitize_text_field( $_GET['group_id'] );
                        $args['aione_ajax_update'] = 'aione_list_ajax_response_'.sanitize_text_field( $_GET['group_id'] );
                        $delete_id_addition = sanitize_text_field( $_GET['group_id'] );
                    } else if( isset( $_GET['aione-tax'] ) ) {
                        $args['aione-tax'] = sanitize_text_field( $_GET['aione-tax'] );
                        $args['aione_ajax_update'] = 'aione_list_ajax_response_'.sanitize_text_field( $_GET['aione-tax'] );
                        $delete_id_addition = sanitize_text_field( $_GET['aione-tax'] );
                    }

                    $args['aione_ref'] = isset( $_GET['ref'] )
                        ? $_GET['ref']
                        : 'list';
                    ...
                    $form['delete'] = array(
                        '#type' => 'markup',
                        '#markup' => sprintf(
                            '<div id="delete-action"><a href="%s" class="submitdelete aione-ajax-link aione-group-delete-link" id="aione-list-delete-%d"">%s</a></div>',
                            esc_url(
                                add_query_arg(
                                    $args,
            ...
            $form['submit'] = array(
                '#type' => 'submit',
                '#name' => 'aione-submit', // do not use only 'submit', because there is JQuery bug which prevents using form.submit() if input submit has name='submit'
                '#value' => $button_text,
                '#attributes' => array(
                    'class' => 'button-primary aione-disabled-on-submit',
                ),
                '_builtin' => true,
            );
            /**
    ...
    {
        if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'aione-edit-tax' ) {
            $this->print_notice( __('These options are not available for built-in taxonomies.', 'aione-app-builder'), 'add-aione-inside');
        } else {
            $this->print_notice( __('These options are not available for built-in post types.', 'aione-app-builder'), 'add-aione-inside');
        }
    }
    ...
    protected function filter_wrap( $type, $data = array(), $button_only = false, $render_closing_hr = true )
    {
        ...
        $form = array();
        $unique_id = aione_unique_id( serialize( func_get_args() ) );
        ...
        $form[$unique_id.'-open'] = array(
            '#type' => 'markup',
            '#markup' => sprintf(
                '<div id="%s" class="aione-filter-container js-aione-filter-container">',
                esc_attr($unique_id)
            ),
        );
        ...
        if( ! $button_only )
            $form[$unique_id.'-content'] = array(
                '#type' => 'markup',
                '#markup' => sprintf(
                    '<span class="js-aione-filter-ajax-response">%s</span>',
        ...
        if ( $this->current_user_can_edit ) {
            $form[$unique_id.'-button'] = array(
                ...
                '#value' => __('Edit', 'aione-app-builder'),
                '#attributes' => array(
                    'class' => 'js-aione-filter-button-edit aione-filter-button-edit',
                    'data-aione-type' => esc_attr($type),
                    'data-aione-page' => esc_attr( aione_getget( 'page' ) ),
                    'data-aione-nonce' => wp_create_nonce($type),
                ),
                '#inline' => true,
                '#before' => '<div class="aione-filter-button-edit-container">',
                '#after' => '</div>',
            );
            foreach( $data as $key => $value ) {
                if ( preg_match('/^data\-aione\-/', $key) ) {
    ...
    protected function verification_failed_and_die($int = 0, $message = false)
    {
        if ( $message ) {
            ...
            _e('Verification failed.', 'aione-app-builder');
            if ($int && intval($int)) {
                printf(' (%d)', $int);
            }
        }
    ...
    function _aione_filter_wrap($id)
    {
        return array(
            $id => array(
                '#type' => 'markup',
    ...
    protected function get_type_from_request($request_name)
    {
        ...
        if (isset($_REQUEST[$request_name]) ) {
            $post_type_option = new get_components();
    ...
    {
        $form = array();
        if ( 'add-aione-inside' == $add_wrap ) {
            $form['wrap-begin'] = array(
                '#type' => 'markup',
                '#markup' => '<div class="aione-inside">',
            );
        }
        $form['alert'] = array(
            '#type' => 'notice',
