...
class Aione_App_Builder_Admin_Custom_Taxonomies_List_Table extends WP_List_Table{
	...
	function __construct()
    {
        ...
        $this->custom_taxonomies = get_option('aione-custom-taxonomies', array());
    ...
    function prepare_items()
    {
        ...
        $data = array(); 
        //echo "=================================";echo "<pre>";print_r($this->custom_taxonomies);echo "</pre>";
        if ( !empty($this->custom_taxonomies) ){
            ...
            foreach( $this->custom_taxonomies as $slug => $taxonomy ) {
                ...
                $one = array(
                    'description' => isset($taxonomy['description'])? $taxonomy['description']:'',
                    'supports' => $supports,
                    'slug' => $taxonomy['slug'],
                    'status' => isset($taxonomy['disabled'])? 'inactive':'active',
                    'title' => stripslashes($taxonomy['labels']['name']),
                    'type' => 'cpt',
                    '_builtin' => false,
                    'author' => isset($taxonomy['_wpcf_author_id'])? intval($taxonomy['_wpcf_author_id']):0,
                );
                $add_one = true;
                if ( $s ) {
                    $add_one = false;
            ...
            $a = array(
                'delete'     => sprintf(
                    ...
                    esc_url(
                        add_query_arg(
                            array(
                                'action' => 'aione_cpt_delete_taxonomy',
                                'wpcf-taxonomy' => $item['slug'],
