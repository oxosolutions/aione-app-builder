 ...
 *
 */
class Aione_App_Builder_Admin_Components_List_Table extends WP_List_Table{
    var $components;
    var $buildin_components;
    var $bulk_action_field_name = 'aione_component_ids';

    function __construct()    {
        //Set parent defaults
        parent::__construct( array(
            'singular'  => 'Component',     //singular name of the listed records
            'plural'    => 'Components',    //plural name of the listed records
            'ajax'      => true        //does this table support ajax?
        ) );

        $post_type_option = new Aione_App_Builder_Admin_Components_Utils();
        $this->components = $post_type_option->get_components();
        $this->buildin_components = aione_get_builtin_in_post_types();
    }
            ...
            case 'taxonomies':
                ...
                if (!empty($item[$column_name])) {
                    foreach ($item[$column_name] as $taxonomy_slug => $taxonomy_name) {
                        $rows[] = stripslashes($taxonomy_name);
                    }
                }
                return empty($rows)? __('None', 'aione-app-builder'):implode(', ', $rows);
            case 'status':
                return 'active' == $item[$column_name]? __('Yes', 'aione-app-builder'):__('No', 'aione-app-builder');
            default:
    ...
    function column_title($item)
    {
        $edit_link = esc_url(
            add_query_arg(
                array(
                    ...
                    'aione-component-slug' => $item['slug'],
                ),
                admin_url('admin.php')
            )
        );
        ...
        //Build row actions
        $actions = array();
        $actions['edit'] = sprintf('<a href="%s">%s</a>', $edit_link, __('Edit', 'aione-app-builder'));
        if ( 'component' == $item['type'] ) {
            $a = array(
                ...
                'delete'     => sprintf(
                    '<a href="%s" class="submitdelete aione-ajax-link" id="aione-list-delete-%s">%s</a>',
                    esc_url(
                        add_query_arg(
                            array(
                                'action' => 'aione_ajax_delete_component',
                                'aione_action' => 'delete_component',
                                'aione-component-slug' => $item['slug'],
                                'aione_ajax_update' => 'aione_list_ajax_response_'.$item['slug'],
                                '_wpnonce' => wp_create_nonce('delete_component'),
            ...
            $actions += $a;
        } elseif ( !aione_is_builtin_post_types( $item['slug'] ) ) {
            $actions = array(
                'view' => sprintf('<a href="%s">%s</a>', $edit_link, __('View', 'aione-app-builder')),
            );
        }
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'title'       => __('Name', 'aione-app-builder'),
            'description' => __('Description', 'aione-app-builder'),
            'status'      => __('Active', 'aione-app-builder'),
            'taxonomies'  => __('Taxonomies', 'aione-app-builder'),
        );
        return $columns;
    }
    /** ************************************************************************
    ...
    {
        $actions = array(
            //'activate'   => __('Activate', 'aione-app-builder'),
            //'deactivate' => __('Deactivate', 'aione-app-builder'),
            'delete'     => __('Delete', 'aione-app-builder'),
        );
        return $actions;
    }
    ...
    function process_bulk_action()
    {
        ...
        if (!empty($action)) {
            ...
            if ( ! wp_verify_nonce( $nonce, 'bulk-components' ) ) {
                die( 'Security check' );
            }
        }
        ...
        if (
             ...
             * update components
             */
            update_option(AIONE_OPTION_NAME_COMPONENTS, $this->components);
            /**
    ...
    function prepare_items()
    {
         ...
         */
        $taxonomies = aione_get_builtin_in_taxonomies('objects');
        ...
        if ( !empty($this->components) ){
            foreach( array_values($this->components) as $type ) {
         ...
         * built-in components
         */
        foreach( $this->buildin_components as $type ) {
            $pt = get_post_type_object($type);
            $one = array(
                'description' => __('This is built-in WordPress Post Type.', 'aione-app-builder'),
         ...
         */
        usort($data, 'aione_usort_reorder');
    ...
    {
        if ( isset($_POST['s']) ) {
            _e('No component found.','aione-app-builder');
            return;
        }
        printf(
