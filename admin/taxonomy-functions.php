 ...
 * @return array
 */
function aione_custom_taxonomies_default() {
    return array(
        'slug' => '',
        'description' => '',
        'supports' => array(),
 ...
 */
function aione_custom_taxonomies_init()
{
    ...
    if ( !empty( $custom_taxonomies ) ) {
        foreach ( $custom_taxonomies as $taxonomy => $data ) {
            if ( 
                ...
                aione_custom_taxonomies_register( $taxonomy, $data );
            }
        }
    }
}
function aione_builtin_taxonomies_init() {
    $custom_taxonomies = get_option( AIONE_OPTION_NAME_TAXONOMIES, array() );
    if ( !empty( $custom_taxonomies ) ) {
        foreach ( $custom_taxonomies as $taxonomy => $data ) {
            if ( isset($data['_builtin']) && $data['_builtin']) {
                aione_taxonomies_register( $taxonomy, $data );
            }
        }
    }
}
 ...
 *
 */
function aione_taxonomies_register($taxonomy, $data)
{
 ...
 * @since unknown
 */
function aione_custom_taxonomies_register( $taxonomy, $data ) {
    ...
    $data = apply_filters( 'aione_taxonomy', $data, $taxonomy );
 ...
 * @return array
 */
function aione_get_active_custom_taxonomies() {
    $taxonomies = get_option(AIONE_OPTION_NAME_TAXONOMIES, array());
