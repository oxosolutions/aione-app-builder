...
if(!class_exists('Aione_Validate')) {
	class Aione_Validate
	{
	    ...
	    public static function check($args, $value)
	    {
	        ...
	        if (is_null(self::$_validation_object)) {
	            self::$_validation_object = new Aione_Cake_Validation();
	        }
	        ...
	        foreach ($args as $method => $v) {
	            ...
	            if (is_callable(array('Aione_Validate', $method))) {
	                $check = call_user_func_array(array('Aione_Validate', $method),
	                        array($v, $value));
	                // Use CakePHP method
	            } else if ((isset(self::$_cake_aliases[$method])
	                    && is_callable(array('Aione_Cake_Validation', self::$_cake_aliases[$method])))
	                    || is_callable(array('Aione_Cake_Validation', $method))) {
	                ...
	                // Validate
	                if (isset(self::$_cake_aliases[$method]) && is_callable(array('Aione_Cake_Validation', self::$_cake_aliases[$method]))) {
	...
	//                                    array_values($v));
	    ...
	    {
	        return (is_callable(array('Aione_Validate',  $method))
	    ...
	    {
	        return is_callable(array('Aione_Validate', $method . '_form'));
	    }
	    ...
	    private static function _set_messages()
	    {
	        ...
	        self::$messages = aione_admin_validation_messages();
	    }
	    ...
	    public static function required_form($field, $data = array())
	    {
		    ...
		    $form['required-checkbox'] = self::merge_form_with_field_settings(
			    array(
				    '#type' => 'checkbox',
				    '#title' => __('Required', 'aione-app-builder'),
	    ...
	    public static function email_form($field, $data = array())
	    {
	        ...
	        $form['email-checkbox'] = self::merge_form_with_field_settings(
		        array(
			        '#type' => 'checkbox',
			        '#title' => __('Email', 'aione-app-builder'),
			        '#name' => $field['#name'] . '[active]',
			        '#default_value' => isset($data['active']) ? 1 : 0,
			        '#inline' => true,
			        '#suffix' => '<br />',
	    ...
	    public static function date_form($field, $data = array())
	    {
	        ...
	        $form['date-checkbox'] = self::merge_form_with_field_settings(
		        array(
		            '#type' => 'checkbox',
		            '#title' => __('Date', 'aione-app-builder'),
		            '#name' => $field['#name'] . '[active]',
		            '#default_value' => isset($data['active']) ? 1 : 0,
		            '#inline' => true,
		            '#suffix' => '<br />',
	    ...
	    public static function digits_form($field, $data = array())
	    {
	        ...
	        $form['digits-checkbox'] = array(
	            '#type' => 'checkbox',
	            '#title' => __('Digits', 'aione-app-builder'),
	            '#name' => $field['#name'] . '[active]',
	    ...
	    public static function number_form($field, $data = array())
	    {
	        ...
	        $form['number-checkbox'] = self::merge_form_with_field_settings(
		        array(
		            '#type' => 'checkbox',
		            '#title' => __('Numeric', 'aione-app-builder'),
		            '#name' => $field['#name'] . '[active]',
		            '#default_value' => $default_value,
		            '#inline' => true,
		            '#suffix' => '<br />',
		...
		public static function skype_form( $field, $data = array() )
		{
			...
			$form['skype-checkbox'] = self::merge_form_with_field_settings(
				array(
					'#type' => 'checkbox',
					'#title' => __('Validation', 'aione-app-builder' ),
					'#label' => __('Skype', 'aione-app-builder'),
					'#name' => $field['#name'] . '[active]',
					'#default_value' => isset( $data['active'] ) ? 1 : 0,
					'#inline' => true,
				),
...
if ( !function_exists( 'array_replace_recursive' ) ) {
	function aione_recurse( $array, $array1 ) {
	...
	function array_replace_recursive( $array, $array1 ) {
		...
		for ($i = 1; $i < count($args); $i++)
		{
			if (is_array($args[$i]))
			{
				$array = aione_recurse($array, $args[$i]);
			}
		}
		return $array;
	}
