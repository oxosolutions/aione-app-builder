 ...
 *
 */
//require_once dirname( __FILE__ ) . '/class-components-utils.php';
add_action( 'aione_type', 'aione_filter_type', 10, 2 );
    ...
    if ( !empty( $custom_types ) ) {
        foreach ( $custom_types as $post_type => $data ) {
            ...
            if (
                ...
                || aione_is_builtin_post_types($post_type)
            ) {
                continue;
            }
            aione_custom_types_register( $post_type, $data );
        }
    }

    // rearrange menu items
    add_filter( 'custom_menu_order' , '__return_true');
    add_filter( 'menu_order', 'aione_menu_order' );
    // rearrange menu items - end

    /** This filter is documented in wp-admin/wp-admin/edit-form-advanced.php */
    add_filter('enter_title_here', 'aione_filter_enter_title_here', 10, 2);
}
...
function aione_menu_order( $menu ) {
	...
	if ( !empty( $custom_types ) ) {
		foreach( $custom_types as $post_type => $data ) {
            ...
            aione_menu_order_item_sort( $menu, $current_url, $target_url );
            ...
            if( isset( $reordered[$current_url] ) ) {
                foreach( $reordered[$current_url] as $post_type ) {
                    aione_menu_order_item_sort( $menu, $post_type['current_url'], $post_type['menu_position'] );
                }
 ...
 * @return mixed
 */
function aione_menu_order_item_sort( &$menu, $current_url, $target_url ) {
    ...
    if( $current_index && $add_menu_after_index )
        aione_custom_types_menu_order_move( $menu, $current_index, $add_menu_after_index );return $menu;
}
 ...
 * @return boolean 
 */
function aione_filter_type( $data, $post_type ) {
    if ( !empty( $data['rewrite']['enabled'] ) ) {
        $data['rewrite']['with_front'] = !empty( $data['rewrite']['with_front'] );
        $data['rewrite']['feeds'] = !empty( $data['rewrite']['feeds'] );
        $data['rewrite']['pages'] = !empty( $data['rewrite']['pages'] );
...
add_filter('dashboard_glance_items', 'aione_dashboard_glance_items');
 ...
 *
 */
function aione_dashboard_glance_items($elements)
{
...
function aione_filter_enter_title_here($enter_title_here, $post)
{
    ...
    if ( empty($enter_title_here) ) {
        $enter_title_here = __('Enter title here', 'aione-app-builder');
    }
 ...
 */
function aione_get_array_key_search_in_sub( $array, $search ) {
    ...
    if ( !empty( $custom_types ) ) {
        ...
        foreach( $custom_types as $post_type => $data ) {
            if(
                ...
                // find menu key
                $post_menu_key = aione_get_array_key_search_in_sub( $menu, $post_edit_page );
                ...
                if( isset( $submenu[$post_edit_page] ) && $post_edit_page != 'upload.php' ) {
                    ...
                    $submenu_overview_key = aione_get_array_key_search_in_sub( $submenu[$post_edit_page], $post_edit_page );

                    if( $submenu_overview_key )
                        $submenu[$post_edit_page][$submenu_overview_key][0] = $data['labels']['name'];

                    // post/page rename add new
                    $submenu_new_key = aione_get_array_key_search_in_sub( $submenu[$post_edit_page], $post_new_page );
...
add_action( 'admin_menu', 'aione_rename_build_in_post_types_menu' );
...
add_action( 'init', 'aione_rename_build_in_post_types' );
...
add_action( 'admin_menu', 'aione_visibility_build_in_types' );